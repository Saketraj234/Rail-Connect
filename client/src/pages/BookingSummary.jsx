...
import { CheckCircle2, Train, Wifi, IndianRupee, MapPin, Calendar, Clock, User, Download, Share2, LayoutDashboard, Printer, Key, Radio, Signal, SignalLow, SignalHigh, WifiOff, AlertTriangle } from 'lucide-react';
...
const BookingSummary = () => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
      ...
      <p className="text-slate-500 font-bold animate-pulse">Fetching your secure ticket...</p>
    </div>
  );
  
  if (!booking) return <div className="text-center p-20 text-red-500 font-bold">Booking not found.</div>;
    ...
    <div className="max-w-5xl mx-auto space-y-12 px-4 pb-20">
      ...
      <div className="text-center space-y-6 pt-10 no-print">
        ...
        <div className="space-y-2">
          ...
          <p className="text-lg text-slate-500 dark:text-slate-400 font-bold">Confirmed for {booking.passengers.length} passenger(s).</p>
        </div>
        
        {/* Journey & WiFi Status Bar */}
        ...
        <div className="flex items-center justify-center gap-4">
          <button 
            ...
            className="flex items-center gap-2 px-8 py-4 bg-slate-900 text-white dark:bg-white dark:text-slate-900 rounded-2xl font-black hover:scale-105 transition-all shadow-lg"
          >
      ...
      <div 
        ref={ticketRef}
        className="relative bg-white text-slate-900 overflow-hidden border border-slate-300 print:shadow-none print:border-slate-400 print:rounded-none mx-auto max-w-[800px] p-8 font-sans"
      >
        {/* IRCTC Style Header */}
        <div className="flex justify-between items-start border-b-2 border-slate-800 pb-4 mb-6">
          <div className="flex gap-4 items-center">
            <div className="flex flex-col items-center">
              <div className="w-16 h-16 bg-blue-900 rounded-full flex items-center justify-center text-white font-bold text-xl">IR</div>
              <span className="text-[8px] font-bold text-blue-900 mt-1 uppercase">Indian Railways</span>
            </div>
          </div>
          <div className="text-center">
            <h1 className="text-xl font-black underline uppercase tracking-tight">Electronic Reservation Slip (ERS)</h1>
            <p className="text-[10px] font-bold text-slate-500 mt-1">Normal User</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-16 h-16 bg-slate-100 rounded-xl flex items-center justify-center text-primary-600 font-black text-2xl border-2 border-primary-600/20">RC</div>
            <span className="text-[8px] font-bold text-primary-600 mt-1 uppercase">RailConnect</span>
          </div>
        </div>

        {/* Journey Details IRCTC Format */}
        <div className="grid grid-cols-3 gap-4 mb-8 items-center">
          <div className="text-center space-y-1">
            <p className="text-xs font-black uppercase text-slate-500">Booked From</p>
            <h4 className="text-sm font-black text-blue-900 uppercase">{booking.train?.source || 'N/A'}</h4>
            <p className="text-[10px] font-bold">Start Date* {booking.travelDate}</p>
          </div>
          
          <div className="relative flex flex-col items-center justify-center">
             <div className="bg-blue-100 text-blue-800 px-4 py-1 rounded-sm text-[10px] font-black uppercase mb-1 relative z-10">Boarding At</div>
             <div className="w-full h-4 bg-blue-400/30 rounded-full relative overflow-hidden">
                <div className="absolute top-0 right-0 w-4 h-4 bg-blue-500 transform rotate-45 translate-x-2"></div>
             </div>
             <p className="text-[10px] font-bold mt-1">Departure* {booking.train?.timing || 'N/A'}</p>
          </div>

          <div className="text-center space-y-1">
            <p className="text-xs font-black uppercase text-slate-500">To</p>
            <h4 className="text-sm font-black text-blue-900 uppercase">{booking.train?.destination || 'N/A'}</h4>
            <p className="text-[10px] font-bold">Arrival* N.A.</p>
          </div>
        </div>

        {/* PNR and Train Info Bar */}
        <div className="grid grid-cols-3 border-y-2 border-slate-800 py-3 mb-6 bg-slate-50">
          <div className="text-center border-r border-slate-300">
            <p className="text-[10px] font-black uppercase text-slate-500">PNR</p>
            <p className="text-lg font-black text-blue-600">{booking._id.slice(-10).toUpperCase()}</p>
            <div className="mt-1">
          ...
          <div className="text-center">
            ...
            <p className="text-sm font-black text-blue-900 uppercase">
              {booking.passengers[0]?.coachNumber?.split('-')[0] || 'SL'} CLASS
            </p>
            <div className="mt-1">
            ...
            <tbody className="font-bold">
              {booking.passengers.map((p, idx) => (
                <tr key={idx} className="border-b border-slate-100">
                  <td className="py-2">{idx + 1}. {p.name.toUpperCase()}</td>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-[10px] font-bold">
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  ...
                  <span>₹ {(booking.totalPrice - (booking.totalPrice * 0.07 + 11.80 + 0.45 * booking.passengers.length)).toFixed(2)}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  <span>WiFi Add-ons</span>
                  <span>₹ {booking.passengers.filter(p => p.wifiSelected).length * 150}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  <span>GST (5%)</span>
                ...
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  ...
                  <span>₹ {(booking.totalPrice * 0.02).toFixed(2)}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-1">
                ...
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  ...
                  <span>₹ {(0.45 * booking.passengers.length).toFixed(2)}</span>
                </div>
              ...
              <p className="text-[8px] font-bold text-slate-400 uppercase mt-2">PG Charges as applicable (Additional)</p>
            </div>
          </div>

          <div className="flex flex-col items-end">
            <div className="w-32 h-32 border-2 border-slate-200 p-2 bg-white mb-2 shadow-sm">
              <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=PNR_${booking._id.slice(-10).toUpperCase()}`} alt="QR Code" className="w-full h-full" />
            </div>
        ...
        {/* IRCTC Style Important Instructions */}
      ...
      <div className="flex items-center justify-center gap-6 no-print">
        <Link to="/" className="px-8 py-4 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-2xl font-black hover:bg-slate-200 transition-all">
          Book Another
        </Link>
